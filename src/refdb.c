...
int git_refdb_lookup(git_refdb *db, git_reference *ref)
{
	...
	return db->backend->lookup(db->backend, ref);
}
...
int git_refdb_delete(struct git_refdb *db, git_reference *ref)
{
	assert(db && db->backend);
...
int git_refdb_packall(struct git_refdb *db)
{
	assert(db && db->backend);

	if (db->backend->packall)
		return db->backend->packall(db->backend);
