...
/*
static void dump_merge_index(git_merge_index *merge_index)
{
    ...
    size_t treediff_conflict_data_len)
{
	...
	cl_git_pass(git_merge_index__find_renames(repo, merge_index, &opts));

	/*
	dump_merge_index(merge_index);
	 */
	
    cl_assert(treediff_conflict_data_len == merge_index->conflicts.length);
