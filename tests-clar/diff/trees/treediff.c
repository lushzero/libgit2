...
void test_diff_trees_treediff__initialize(void)
{
	repo = cl_git_sandbox_init(TEST_REPO_PATH);
}

void test_diff_trees_treediff__cleanup(void)
{
...
static int treediff_cb(const git_diff_tree_delta *delta, void *cb_data)
{
    ...
    for (i = 0; i < treediff_data->trees_len; i++) {
        if (treediff_data->file_data[idx+i].mode == 0) {
            ...
            if (strcmp(treediff_data->file_data[idx+i].path, delta->files[i].path) != 0 ||
                git_oid_cmp(&oid, &delta->files[i].oid) != 0)
                return -1;
        }
    }
...
static git_diff_tree_list *treediff(
    ...
    git_diff_tree_options *opts)
{
    ...
    cl_assert(oids = git__calloc(trees_len, sizeof(git_oid)));
    cl_assert(trees = git__calloc(trees_len, sizeof(git_tree *)));
    ...
    cl_git_pass(git_diff_trees(&diff_tree, repo, trees, trees_len, opts));
    
    cl_assert(treediff_file_data_len == diff_tree->deltas.length);
    
    treediff_data.trees_len = trees_len;
...
void test_diff_trees_treediff__two_trees(void)
{
    git_diff_tree_list *diff_tree;
    const char *tree_oid_strs[] = { TREE_OID_ONE, TREE_OID_TWO };
    git_diff_tree_options opts = {0};
    ...
    cl_assert(diff_tree = treediff(tree_oid_strs, 2, treediff_file_data, 7, &opts));
    
    git_diff_tree_list_free(diff_tree);
}

void test_diff_trees_treediff__two_trees_unmodified(void)
{
    git_diff_tree_list *diff_tree;
    const char *tree_oid_strs[] = { TREE_OID_ONE, TREE_OID_TWO };
    git_diff_tree_options opts = {0};
    
    opts.flags = GIT_DIFF_TREE_RETURN_UNMODIFIED;
    ...
    cl_assert(diff_tree = treediff(tree_oid_strs, 2, treediff_file_data, 8, &opts));
    
    git_diff_tree_list_free(diff_tree);
}

void test_diff_trees_treediff__three_trees(void)
{
    git_diff_tree_list *diff_tree;
    ...
    cl_assert(diff_tree = treediff(tree_oid_strs, 3, treediff_file_data, 7, &opts));
    
    git_diff_tree_list_free(diff_tree);
}

void test_diff_trees_treediff__three_trees_unmodified(void)
{
    git_diff_tree_list *diff_tree;
    const char *tree_oid_strs[] = { TREE_OID_ANCESTOR, TREE_OID_ONE, TREE_OID_TWO };
    git_diff_tree_options opts = {0};
    ...
    cl_assert(diff_tree = treediff(tree_oid_strs, 3, treediff_file_data, 8, &opts));
    
    git_diff_tree_list_free(diff_tree);
}

void test_diff_trees_treediff__six_trees(void)
{
    git_diff_tree_list *diff_tree;
    const char *tree_oid_strs[] = {
    ...
    cl_assert(diff_tree = treediff(tree_oid_strs, 6, treediff_file_data, 6, &opts));
